...
class OAuth1Session(requests.Session):
    ...
    def authorized(self):
        ...
        if self._client.client.signature_method == SIGNATURE_RSA:
            ...
            return (bool(self._client.client.client_secret)
    ...
    def authorization_url(self, url, request_token=None, **kwargs):
        ...
        """
        kwargs[
            "oauth_token"] = request_token or self._client.client.resource_owner_key
    ...
    def fetch_request_token(self, url, realm=None, **request_kwargs):
        ...
        log.debug(
            "Resetting callback_uri and realm (not needed in next phase).")
    ...
    def _populate_attributes(self, token):
        ...
        if "oauth_token_secret" in token:
            self._client.client.resource_owner_secret = token[
                "oauth_token_secret"]
        if "oauth_verifier" in token:
            self._client.client.verifier = token["oauth_verifier"]

    def _fetch_token(self, url, **request_kwargs):
        log.debug("Fetching token from %s using client %s", url,
                  self._client.client)
