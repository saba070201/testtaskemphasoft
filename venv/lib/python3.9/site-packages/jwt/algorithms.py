...
if TYPE_CHECKING:
    ...
    AllowedOKPKeys = (Ed25519PrivateKey | Ed25519PublicKey | Ed448PrivateKey
                      | Ed448PublicKey)
...
class Algorithm(ABC):
    ...
    def compute_hash_digest(self, bytestr: bytes) -> bytes:
        ...
        if (has_crypto and isinstance(hash_alg, type)
...
class HMACAlgorithm(Algorithm):
    ...
    @staticmethod
    def to_jwk(key_obj: str | bytes,
               as_dict: bool = False) -> Union[JWKDict, str]:
        jwk = {
...
if has_crypto:
    ...
    class RSAAlgorithm(Algorithm):
        ...
        def prepare_key(self,
                        key: AllowedRSAKeys | str | bytes) -> AllowedRSAKeys:
        ...
        def to_jwk(key_obj: AllowedRSAKeys,
                   as_dict: Literal[False] = False) -> str:
            ...  # pragma: no cover
    ...
    class ECAlgorithm(Algorithm):
        ...
        def prepare_key(self,
                        key: AllowedECKeys | str | bytes) -> AllowedECKeys:
            if isinstance(key,
        ...
        def verify(self, msg: bytes, key: "AllowedECKeys", sig: bytes) -> bool:
            ...
            try:
                public_key = (key.public_key() if isinstance(
                    key, EllipticCurvePrivateKey) else key)
        ...
        @overload
        @staticmethod
        def to_jwk(key_obj: AllowedECKeys,
                   as_dict: Literal[False] = False) -> str:
            ...  # pragma: no cover
        ...
        def to_jwk(key_obj: AllowedECKeys,
            ...
            if isinstance(key_obj, EllipticCurvePrivateKey):
                obj["d"] = to_base64url_uint(
                    key_obj.private_numbers().private_value).decode()
    ...
    class OKPAlgorithm(Algorithm):
        ...
        def prepare_key(self,
                        key: AllowedOKPKeys | str | bytes) -> AllowedOKPKeys:
            if isinstance(key, (bytes, str)):
                key_str = key.decode("utf-8") if isinstance(key,
                                                            bytes) else key
                key_bytes = key.encode("utf-8") if isinstance(key,
                                                              str) else key
        ...
        def sign(self, msg: str | bytes,
                 key: Ed25519PrivateKey | Ed448PrivateKey) -> bytes:
            """
        ...
        def verify(self, msg: str | bytes, key: AllowedOKPKeys,
                   sig: str | bytes) -> bool:
            """
            ...
            """
            try:
                msg_bytes = msg.encode("utf-8") if isinstance(msg,
                                                              str) else msg
                sig_bytes = sig.encode("utf-8") if isinstance(sig,
                                                              str) else sig

                public_key = (key.public_key() if isinstance(
                    key, (Ed25519PrivateKey, Ed448PrivateKey)) else key)
        ...
        def to_jwk(key: AllowedOKPKeys,
            ...
            if isinstance(key, (Ed25519PublicKey, Ed448PublicKey)):
                ...
                crv = "Ed25519" if isinstance(key,
                                              Ed25519PublicKey) else "Ed448"
