...
_RAW_TO_EMAIL_MAPPING = {
    raw: email
    for email, raw in _EMAIL_TO_RAW_MAPPING.items()
}


def parse_email(
        data: Union[bytes, str]) -> Tuple[RawMetadata, Dict[str, List[str]]]:
        ...
        if payload:
            ...
            if "description" in raw:
                ...
                unparsed.setdefault("description",
                                    []).extend([description_header, payload])
...
class _Validator(Generic[T]):
    ...
    def _invalid_metadata(
            self,
            msg: str,
            cause: Optional[Exception] = None) -> InvalidMetadata:
        exc = InvalidMetadata(self.raw_name,
                              msg.format_map({"field": repr(self.raw_name)}))
        exc.__cause__ = cause
        return exc
    ...
    def _process_name(self, value: str) -> str:
        ...
        try:
            ...
            raise self._invalid_metadata(f"{value!r} is invalid for {{field}}",
                                         cause=exc)
        else:
            return value
        ...
        try:
            for name in value:
                normalized_names.append(
                    utils.canonicalize_name(name, validate=True))
        except utils.InvalidName as exc:
            raise self._invalid_metadata(f"{name!r} is invalid for {{field}}",
                                         cause=exc)
        else:
    ...
    def _process_requires_dist(
        ...
        try:
            ...
            raise self._invalid_metadata(f"{req!r} is invalid for {{field}}",
                                         cause=exc)
        else:
            return reqs
...
class Metadata:
    ...
    def from_email(cls,
        ...
        try:
            ...
            raise ExceptionGroup("invalid or unparsed metadata",
                                 exceptions) from None
