...
PKCS7HashTypes = typing.Union[hashes.SHA224, hashes.SHA256, hashes.SHA384,
                              hashes.SHA512, ]

PKCS7PrivateKeyTypes = typing.Union[rsa.RSAPrivateKey,
...
class PKCS7SignatureBuilder:
    def __init__(
        ...
        signers: typing.List[typing.Tuple[x509.Certificate,
    ...
    def add_signer(
        ...
        if not isinstance(
            ...
            raise TypeError("hash_algorithm must be one of hashes.SHA224, "
    ...
    def add_certificate(
            self, certificate: x509.Certificate) -> PKCS7SignatureBuilder:
        if not isinstance(certificate, x509.Certificate):
            raise TypeError("certificate must be a x509.Certificate")

        return PKCS7SignatureBuilder(self._data, self._signers,
...
def _smime_encode(data: bytes, signature: bytes, micalg: str,
    ...
    sig_part.add_header("Content-Type",
    ...
    sig_part.add_header("Content-Disposition",
                        "attachment",
                        filename="smime.p7s")
    sig_part.set_payload(email.base64mime.body_encode(signature,
                                                      maxlinelen=65))
