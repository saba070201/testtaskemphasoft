...
class _SSHCipher:
    ...
    key_len: int
    mode: typing.Union[typing.Type[modes.CTR], typing.Type[modes.CBC],
...
def _get_ssh_key_type(
        key: typing.Union[SSHPrivateKeyTypes, SSHPublicKeyTypes]) -> bytes:
...
def _init_cipher(
    ...
    seed = _bcrypt_kdf(password, salt, ciph.key_len + ciph.iv_len, rounds,
                       True)
    return Cipher(
...
class _FragList:
    ...
    def __init__(self,
                 init: typing.Optional[typing.List[bytes]] = None) -> None:
        self.flist = []
        if init:
...
class _SSHFormatRSA:
    ...
    def load_public(
            self,
            data: memoryview) -> typing.Tuple[rsa.RSAPublicKey, memoryview]:
    ...
    def load_private(self, data: memoryview,
                     pubfields) -> typing.Tuple[rsa.RSAPrivateKey, memoryview]:
    ...
    def encode_public(self, public_key: rsa.RSAPublicKey,
...
class _SSHFormatDSA:
    ...
    """
    def get_public(self,
                   data: memoryview) -> typing.Tuple[typing.Tuple, memoryview]:
        """DSA public fields"""
        p, data = _get_mpint(data)
        q, data = _get_mpint(data)
    ...
    def load_public(
            self,
            data: memoryview) -> typing.Tuple[dsa.DSAPublicKey, memoryview]:
    ...
    def encode_public(self, public_key: dsa.DSAPublicKey,
                      f_pub: _FragList) -> None:
    ...
    def encode_private(self, private_key: dsa.DSAPrivateKey,
                       f_priv: _FragList) -> None:
...
class _SSHFormatECDSA:
    ...
    def load_private(
            self, data: memoryview,
            pubfields) -> typing.Tuple[ec.EllipticCurvePrivateKey, memoryview]:
...
class _SSHFormatEd25519:
    ...
    def load_private(
            self, data: memoryview,
            pubfields) -> typing.Tuple[ed25519.Ed25519PrivateKey, memoryview]:
    ...
    def encode_public(self, public_key: ed25519.Ed25519PublicKey,
                      f_pub: _FragList) -> None:
...
SSHPrivateKeyTypes = typing.Union[ec.EllipticCurvePrivateKey,
...
def _serialize_ssh_private_key(
    ...
    if password:
        ...
        if (isinstance(encryption_algorithm, _KeySerializationEncryption)
...
SSHPublicKeyTypes = typing.Union[ec.EllipticCurvePublicKey, rsa.RSAPublicKey,
                                 dsa.DSAPublicKey, ed25519.Ed25519PublicKey, ]

SSHCertPublicKeyTypes = typing.Union[ec.EllipticCurvePublicKey,
                                     rsa.RSAPublicKey,
...
class SSHCertificate:
    ...
    def public_bytes(self) -> bytes:
        return (bytes(self._cert_key_type) + b" " +
    ...
    def verify_cert_signature(self) -> None:
        ...
        if isinstance(signature_key, ed25519.Ed25519PublicKey):
            signature_key.verify(bytes(self._signature),
            ...
            signature_key.verify(computed_sig, bytes(self._tbs_cert_body),
                                 ec.ECDSA(hash_alg))
        else:
...
def _load_ssh_public_identity(
    ...
    if with_cert:
        ...
        if (sig_type == _SSH_RSA and inner_sig_type not in [
                _SSH_RSA_SHA256, _SSH_RSA_SHA512, _SSH_RSA
...
def load_ssh_public_identity(
    data: bytes, ) -> typing.Union[SSHCertificate, SSHPublicKeyTypes]:
...
def load_ssh_public_key(data: bytes,
                        backend: typing.Any = None) -> SSHPublicKeyTypes:
...
SSHCertPrivateKeyTypes = typing.Union[ec.EllipticCurvePrivateKey,
                                      rsa.RSAPrivateKey,
                                      ed25519.Ed25519PrivateKey, ]
...
class SSHCertificateBuilder:
    ...
    def public_key(self,
                   public_key: SSHCertPublicKeyTypes) -> SSHCertificateBuilder:
        if not isinstance(
                public_key,
            (
    ...
    def valid_principals(
            self,
            valid_principals: typing.List[bytes]) -> SSHCertificateBuilder:
    ...
    def valid_for_all_principals(self):
        if self._valid_principals:
            raise ValueError("valid_principals already set, can't set "
    ...
    def valid_before(
            self, valid_before: typing.Union[int,
                                             float]) -> SSHCertificateBuilder:
    ...
    def valid_after(
            self, valid_after: typing.Union[int,
                                            float]) -> SSHCertificateBuilder:
    ...
    def add_critical_option(self, name: bytes,
                            value: bytes) -> SSHCertificateBuilder:
    ...
    def add_extension(self, name: bytes,
                      value: bytes) -> SSHCertificateBuilder:
        if not isinstance(name, bytes) or not isinstance(value, bytes):
            raise TypeError("name and value must be bytes")
        # This is O(n**2)
    ...
    def sign(self, private_key: SSHCertPrivateKeyTypes) -> SSHCertificate:
        ...
        if isinstance(private_key, ed25519.Ed25519PrivateKey):
            ...
            signature = private_key.sign(f.tobytes(), padding.PKCS1v15(),
                                         hashes.SHA512())
            fsig.put_sshstr(signature)
            f.put_sshstr(fsig.tobytes())
