...
Mode = typing.TypeVar("Mode",
                      bound=typing.Optional[modes.Mode],
                      covariant=True)
...
class Cipher(typing.Generic[Mode]):
    ...
    def encryptor(self):
        ...
        ctx = backend.create_symmetric_encryption_ctx(self.algorithm,
                                                      self.mode)
    ...
    def decryptor(self):
        ...
        ctx = backend.create_symmetric_decryption_ctx(self.algorithm,
                                                      self.mode)
        return self._wrap_ctx(ctx, encrypt=False)

    def _wrap_ctx(
        self, ctx: _BackendCipherContext, encrypt: bool
    ) -> typing.Union[AEADEncryptionContext, AEADDecryptionContext,
                      CipherContext]:
        if isinstance(self.mode, modes.ModeWithAuthenticationTag):
            if encrypt:
...
_CIPHER_TYPE = Cipher[typing.Union[modes.ModeWithNonce, modes.ModeWithTweak,
                                   None, modes.ECB,
...
class _AEADCipherContext(AEADCipherContext):
        ...
        if self._bytes_processed > self._ctx._mode._MAX_ENCRYPTED_BYTES:
            raise ValueError(
                "{} has a maximum encrypted byte limit of {}".format(
                    self._ctx._mode.name,
                    self._ctx._mode._MAX_ENCRYPTED_BYTES))
    ...
    def authenticate_additional_data(self, data: bytes) -> None:
        ...
        if self._aad_bytes_processed > self._ctx._mode._MAX_AAD_BYTES:
            raise ValueError("{} has a maximum AAD byte limit of {}".format(
