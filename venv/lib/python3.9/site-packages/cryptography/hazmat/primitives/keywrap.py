...
def _wrap_core(
    ...
    for j in range(6):
        for i in range(n):
            ...
            a = (int.from_bytes(b[:8], byteorder="big") ^
                 ((n * j) + i + 1)).to_bytes(length=8, byteorder="big")
            r[i] = b[-8:]
...
def _unwrap_core(
    ...
    for j in reversed(range(6)):
        for i in reversed(range(n)):
            atr = (int.from_bytes(a, byteorder="big") ^ (
                (n * j) + i + 1)).to_bytes(length=8, byteorder="big") + r[i]
...
def aes_key_wrap_with_padding(
    ...
    aiv = b"\xA6\x59\x59\xA6" + len(key_to_wrap).to_bytes(length=4,
                                                          byteorder="big")
