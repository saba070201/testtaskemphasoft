...
def _enc_dec_rsa_pkey_ctx(
    ...
    if (isinstance(padding, OAEP) and padding._label is not None
...
def _rsa_sig_sign(
    ...
    res = backend._lib.EVP_PKEY_sign(pkey_ctx, backend._ffi.NULL, buflen, data,
                                     len(data))
    backend.openssl_assert(res == 1)
...
def _rsa_sig_verify(
    ...
    res = backend._lib.EVP_PKEY_verify(pkey_ctx, signature, len(signature),
                                       data, len(data))
...
def _rsa_sig_recover(
    ...
    res = backend._lib.EVP_PKEY_verify_recover(pkey_ctx, buf, buflen,
...
class _RSAPrivateKey(RSAPrivateKey):
    ...
    def _non_threadsafe_enable_blinding(self) -> None:
        ...
        if not self._blinded:
            res = self._backend._lib.RSA_blinding_on(self._rsa_cdata,
                                                     self._backend._ffi.NULL)
...
class _RSAPublicKey(RSAPublicKey):
    ...
    def recover_data_from_signature(
        ...
        return _rsa_sig_recover(self._backend, padding, algorithm, self,
                                signature)
