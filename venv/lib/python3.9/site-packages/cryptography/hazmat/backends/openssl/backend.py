...
class Backend:
    ...
    def __init__(self) -> None:
        ...
        self._cipher_registry: typing.Dict[typing.Tuple[
            typing.Type[CipherAlgorithm], typing.Type[Mode]],
                                           typing.Callable, ] = {}
    ...
    def openssl_version_text(self) -> str:
        ...
        return self._ffi.string(
            self._lib.OpenSSL_version(
                self._lib.OPENSSL_VERSION)).decode("ascii")
    ...
    def _evp_md_from_algorithm(self, algorithm: hashes.HashAlgorithm):
        if algorithm.name == "blake2b" or algorithm.name == "blake2s":
            alg = "{}{}".format(algorithm.name,
                                algorithm.digest_size * 8).encode("ascii")
        else:
    ...
    def signature_hash_supported(self,
                                 algorithm: hashes.HashAlgorithm) -> bool:
    ...
    def _register_default_ciphers(self) -> None:
        ...
        self.register_cipher_adapter(TripleDES, ECB,
                                     GetCipherByName("des-ede3"))
        self.register_cipher_adapter(ChaCha20, type(None),
                                     GetCipherByName("chacha20"))
        self.register_cipher_adapter(AES, XTS, _get_xts_cipher)
        for mode_cls in [ECB, CBC, OFB, CFB, CTR]:
            self.register_cipher_adapter(SM4, mode_cls,
                                         GetCipherByName("sm4-{mode.name}"))
        ...
        if (self._binding._legacy_provider_loaded
            ...
            self.register_cipher_adapter(ARC4, type(None),
                                         GetCipherByName("rc4"))
            # We don't actually support RC2, this is just used by some tests.
            self.register_cipher_adapter(_RC2, type(None),
                                         GetCipherByName("rc2"))

    def create_symmetric_encryption_ctx(self, cipher: CipherAlgorithm,
                                        mode: Mode) -> _CipherContext:
        return _CipherContext(self, cipher, mode, _CipherContext._ENCRYPT)

    def create_symmetric_decryption_ctx(self, cipher: CipherAlgorithm,
                                        mode: Mode) -> _CipherContext:
    ...
    def generate_rsa_private_key(self, public_exponent: int,
                                 key_size: int) -> rsa.RSAPrivateKey:
        ...
        return _RSAPrivateKey(self,
                              rsa_cdata,
                              evp_pkey,
                              unsafe_skip_rsa_key_validation=True)

    def generate_rsa_parameters_supported(self, public_exponent: int,
                                          key_size: int) -> bool:
        return (public_exponent >= 3 and public_exponent & 1 != 0
                and key_size >= 512)
    ...
    def load_rsa_public_numbers(
            self, numbers: rsa.RSAPublicNumbers) -> rsa.RSAPublicKey:
    ...
    def rsa_padding_supported(self, padding: AsymmetricPadding) -> bool:
        if isinstance(padding, PKCS1v15):
            ...
            if self._fips_enabled and isinstance(padding._mgf._algorithm,
                                                 hashes.SHA1):
                return True
            else:
            ...
            return self._oaep_hash_supported(
                padding._mgf._algorithm) and self._oaep_hash_supported(
                    padding._algorithm)
        else:
            return False
    ...
    def cmac_algorithm_supported(self, algorithm) -> bool:
        return self.cipher_supported(algorithm,
                                     CBC(b"\x00" * algorithm.block_size))
    ...
    def load_pem_public_key(self, data: bytes) -> PublicKeyTypes:
        ...
        evp_pkey = self._lib.PEM_read_bio_PUBKEY(
            mem_bio.bio,
            self._ffi.NULL,
            self._ffi.addressof(self._lib._original_lib,
    ...
    def _load_key(self, openssl_read_func, data, password,
                  unsafe_skip_rsa_key_validation) -> PrivateKeyTypes:
        mem_bio = self._bytes_to_bio(data)
        ...
        assert (password is not None
                and userdata.called == 1) or password is None
    ...
    def _handle_key_loading_error(self) -> typing.NoReturn:
        ...
        elif (errors[0]._lib_reason_match(self._lib.ERR_LIB_EVP,
                                          self._lib.EVP_R_BAD_DECRYPT)
              or errors[0]._lib_reason_match(
                  self._lib.ERR_LIB_PKCS12,
                  self._lib.PKCS12_R_PKCS12_CIPHERFINAL_ERROR,
              ) or (self._lib.Cryptography_HAS_PROVIDERS
    ...
    def generate_elliptic_curve_private_key(
            self, curve: ec.EllipticCurve) -> ec.EllipticCurvePrivateKey:
        """
    ...
    def load_elliptic_curve_private_numbers(
        ...
        private_value = self._ffi.gc(self._int_to_bn(numbers.private_value),
                                     self._lib.BN_clear_free)
        ...
        with self._tmp_bn_ctx() as bn_ctx:
            ...
            computed_point = self._ffi.gc(computed_point,
                                          self._lib.EC_POINT_free)
            ...
            self.openssl_assert(res == 1)
            if (self._lib.EC_POINT_cmp(group, set_point, computed_point,
                                       bn_ctx) != 0):
                raise ValueError("Invalid EC key.")
    ...
    def load_elliptic_curve_public_bytes(
            ...
            point_bytes: bytes) -> ec.EllipticCurvePublicKey:
        ec_cdata = self._ec_key_new_by_curve(curve)
        group = self._lib.EC_KEY_get0_group(ec_cdata)
        self.openssl_assert(group != self._ffi.NULL)
    ...
    def derive_elliptic_curve_private_key(
            self, private_value: int,
            curve: ec.EllipticCurve) -> ec.EllipticCurvePrivateKey:
        ec_cdata = self._ec_key_new_by_curve(curve)
    ...
    def elliptic_curve_exchange_algorithm_supported(
            self, algorithm: ec.ECDH, curve: ec.EllipticCurve) -> bool:
        if self._fips_enabled and not isinstance(curve,
                                                 self._fips_ecdh_curves):
            return False
    ...
    def _private_key_bytes(
        ...
        if isinstance(encryption_algorithm, serialization.NoEncryption):
            ...
            if len(password) > 1023:
              ...
              and encryption_algorithm._format is format is
        ...
        if format is serialization.PrivateFormat.PKCS8:
            ...
            return self._private_key_bytes_via_bio(write_bio, evp_pkey,
                                                   password)
        ...
        if format is serialization.PrivateFormat.TraditionalOpenSSL:
            if self._fips_enabled and not isinstance(
                ...
                raise ValueError("Encrypted traditional OpenSSL format is not "
            ...
            if encoding is serialization.Encoding.PEM:
                ...
                return self._private_key_bytes_via_bio(write_bio, cdata,
                                                       password)
        ...
        if format is serialization.PrivateFormat.OpenSSH:
            if encoding is serialization.Encoding.PEM:
                return ssh._serialize_ssh_private_key(key, password,
                                                      encryption_algorithm)

            raise ValueError("OpenSSH private key format can only be used"
                             " with PEM encoding")
    ...
    def _private_key_bytes_via_bio(self, write_bio, evp_pkey,
                                   password) -> bytes:
        if not password:
    ...
    def x25519_load_private_bytes(self,
                                  data: bytes) -> x25519.X25519PrivateKey:
    ...
    def ed25519_load_public_bytes(self,
                                  data: bytes) -> ed25519.Ed25519PublicKey:
        return rust_openssl.ed25519.from_public_bytes(data)

    def ed25519_load_private_bytes(self,
                                   data: bytes) -> ed25519.Ed25519PrivateKey:
    ...
    ) -> typing.Tuple[typing.Optional[PrivateKeyTypes],
    ...
    def load_pkcs12(
            self, data: bytes,
            password: typing.Optional[bytes]) -> PKCS12KeyAndCertificates:
        if password is not None:
            utils._check_byteslike("password", password)
        ...
        with self._zeroed_null_terminated_buf(password) as password_buf:
            res = self._lib.PKCS12_parse(p12, password_buf, evp_pkey_ptr,
                                         x509_ptr, sk_x509_ptr)
        if res == 0:
            self._consume_errors()
    ...
    def serialize_key_and_certificates_to_pkcs12(
        ...
        if isinstance(encryption_algorithm, serialization.NoEncryption):
            ...
            password = encryption_algorithm.password
                         ...
                         serialization._KeySerializationEncryption)
              and encryption_algorithm._format is
        ...
        with self._zeroed_null_terminated_buf(password) as password_buf:
            with self._zeroed_null_terminated_buf(name) as name_buf:
                ...
                ossl_pkey = (self._key2ossl(key)
                             if key is not None else self._ffi.NULL)
            ...
            if (self._lib.Cryptography_HAS_PKCS12_SET_MAC
    ...
    def load_pem_pkcs7_certificates(
            self, data: bytes) -> typing.List[x509.Certificate]:
        utils._check_bytes("data", data)
        bio = self._bytes_to_bio(data)
        p7 = self._lib.PEM_read_bio_PKCS7(bio.bio, self._ffi.NULL,
...
class GetCipherByName:
    ...
    def __call__(self, backend: Backend, cipher: CipherAlgorithm, mode: Mode):
        ...
        if (evp_cipher == backend._ffi.NULL
