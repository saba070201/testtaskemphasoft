...
def _check_signature_algorithm(
    signature_algorithm: ec.EllipticCurveSignatureAlgorithm, ) -> None:
...
def _mark_asn1_named_ec_curve(backend: Backend, ec_cdata):
    ...
    backend._lib.EC_KEY_set_asn1_flag(ec_cdata,
                                      backend._lib.OPENSSL_EC_NAMED_CURVE)
...
def _ecdsa_sig_sign(backend: Backend, private_key: _EllipticCurvePrivateKey,
                    data: bytes) -> bytes:
    ...
    res = backend._lib.ECDSA_sign(0, data, len(data), sigbuf, siglen_ptr,
                                  private_key._ec_key)
...
def _ecdsa_sig_verify(
    ...
    res = backend._lib.ECDSA_verify(0, data, len(data), signature,
                                    len(signature), public_key._ec_key)
    if res != 1:
...
class _EllipticCurvePrivateKey(ec.EllipticCurvePrivateKey):
    ...
    def exchange(self, algorithm: ec.ECDH,
...
class _EllipticCurvePublicKey(ec.EllipticCurvePublicKey):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (self._backend._lib.EVP_PKEY_cmp(self._evp_pkey,
                                                other._evp_pkey) == 1)
