...
class GrantTypeBase:
    ...
    def id_token_hash(self, value, hashfunc=hashlib.sha256):
        ...
        return base64.urlsafe_b64encode(
            digest[:left_most]).decode().rstrip("=")
    ...
    def add_id_token(self, token, token_handler, request, nonce=None):
        ...
        id_token = self.request_validator.get_id_token(token, token_handler,
                                                       request)
        if id_token:
        ...
        token['id_token'] = self.request_validator.finalize_id_token(
    ...
    def openid_authorization_validator(self, request):
        ...
        if 'none' in prompt:
            ...
            if not self.request_validator.validate_silent_authorization(
                    request):
        ...
        if not self.request_validator.validate_user_match(
                request.id_token_hint, request.scopes, request.claims,
                request):
        ...
        request_info = {
            ...
            'prompt': prompt,
            'ui_locales':
            request.ui_locales.split() if request.ui_locales else [],
