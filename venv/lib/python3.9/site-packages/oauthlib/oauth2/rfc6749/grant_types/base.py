...
class GrantTypeBase:
        ...
        if not hasattr(self, 'validate_authorization_request'):
            if post_auth or pre_auth:
                msg = ("{} does not support authorization validators. Use "
                       "token validators instead.").format(
    ...
    def add_token(self, token, token_handler, request):
        ...
        if not request.response_type in [
                "token", "code token", "id_token token", "code id_token token"
        ]:
            return token
    ...
    def validate_grant_type(self, request):
        ...
        if not self.request_validator.validate_grant_type(
    ...
    def validate_scopes(self, request):
        ...
        if not request.scopes:
            request.scopes = utils.scope_to_list(
                request.scope) or utils.scope_to_list(
                    self.request_validator.get_default_scopes(
    ...
    def prepare_authorization_response(self, request, token, headers, body,
                                       status):
        ...
        if request.response_mode == 'query':
            headers['Location'] = add_params_to_uri(request.redirect_uri,
                                                    token_items,
                                                    fragment=False)
