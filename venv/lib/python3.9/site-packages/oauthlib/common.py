...
def encode_params_utf8(params):
    ...
    encoded = []
    for k, v in params:
        encoded.append((k.encode('utf-8') if isinstance(k, str) else k,
...
def decode_params_utf8(params):
    ...
    decoded = []
    for k, v in params:
        decoded.append((k.decode('utf-8') if isinstance(k, bytes) else k,
    ...
    if hasattr(data, '__iter__'):
        try:
            ...
            if hasattr(data, 'items'):
                data = data.items()
            return {
                to_unicode(k, encoding): to_unicode(v, encoding)
                for k, v in data
            }
    ...
    * list of 2-tuples
    ...
    def uri_query_params(self):
        if not self.uri_query:
            return []
        return urlparse.parse_qsl(self.uri_query,
