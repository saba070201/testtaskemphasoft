...
def memorized_ttinfo(*args):
    ...
    try:
        ...
        ttinfo = (memorized_timedelta(args[0]), memorized_timedelta(args[1]),
                  args[2])
...
class DstTzInfo(BaseTzInfo):
    ...
    def __repr__(self):
        ...
        if self._utcoffset > _notime:
            return '<DstTzInfo %r %s+%s %s>' % (self.zone, self._tzname,
                                                self._utcoffset, dst)
        else:
            return '<DstTzInfo %r %s%s %s>' % (self.zone, self._tzname,
                                               self._utcoffset, dst)
    ...
    def __reduce__(self):
        # Special pickle to zone remains a singleton and to cope with
        # database changes.
        return pytz._p, (self.zone, _to_seconds(self._utcoffset),
                         _to_seconds(self._dst), self._tzname)
