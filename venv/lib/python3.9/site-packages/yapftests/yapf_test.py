...
class CommandLineTest(yapf_test_helper.YAPFTest):
    ...
    def testInPlaceReformattingBlank(self):
        ...
        with utils.TempFileContents(self.test_tmpdir,
            ...
            p = subprocess.Popen(YAPF_BINARY + ['--in-place', filepath])
            p.wait()
            with io.open(filepath, mode='r', encoding='utf-8',
                         newline='') as fd:
                reformatted_code = fd.read()
        self.assertEqual(reformatted_code, expected_formatted_code)
    ...
    def testPrintModified(self):
        for unformatted_code, has_change in [('1==2', True),
                                             ('1 == 2', False)]:
            with utils.TempFileContents(self.test_tmpdir,
                                        unformatted_code,
                                        suffix='.py') as filepath:
    ...
    """)
        ...
        with utils.NamedTempFile(suffix='.py',
                                 dirname=self.test_tmpdir) as (out, _):
            with utils.TempFileContents(self.test_tmpdir,
                                        unformatted_code,
                                        suffix='.py') as filepath:
                try:
                    subprocess.check_call(YAPF_BINARY + ['--diff', filepath],
                                          stdout=out)
...
class DiffIndentTest(yapf_test_helper.YAPFTest):
    ...
    def _Check(self, unformatted_code, expected_formatted_code):
        formatted_code, _ = yapf_api.FormatCode(
            unformatted_code,
            style_config=style.SetGlobalStyle(self._OwnStyle()))
