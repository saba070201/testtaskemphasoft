...
class PytreeUnwrapperTest(yapf_test_helper.YAPFTest):
    ...
    """
        ...
        for lline in llines:
            filtered_values = [
                ft.value for ft in lline.tokens
...
class MatchBracketsTest(yapf_test_helper.YAPFTest):
    ...
    """
        ...
        for lline in llines:
            filtered_values = [
                (ft, ft.matching_bracket) for ft in lline.tokens
                if ft.name not in pytree_utils.NONSEMANTIC_TOKENS
            ]
            if filtered_values:
        ...
        for index, bracket_list in enumerate(list_of_expected):
            ...
            if not bracket_list:
                ...
                for open_bracket, close_bracket in bracket_list:
                    self.assertEqual(lline[open_bracket][0],
                                     lline[close_bracket][1])
                    self.assertEqual(lline[close_bracket][0],
                                     lline[open_bracket][1])
