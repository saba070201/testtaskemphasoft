...
class GetExcludePatternsForDir(yapf_test_helper.YAPFTest):
    ...
    def test_get_exclude_file_patterns_from_pyproject(self):
        ...
        with open(local_ignore_file, 'w') as f:
            ...
            f.writelines('\n,'.join(
                ['"{}"'.format(p) for p in ignore_patterns]))
            f.write(']')
    ...
    def test_get_exclude_file_patterns_from_pyproject_ignore_section_empty(
            self):
        try:
            import tomli
        except ImportError:
...
class GetCommandLineFilesTest(yapf_test_helper.YAPFTest):
    ...
    def test_find_with_excluded_dirs(self):
        ...
        self.assertEqual(
            found,
            ['test3/foo/bar/bas/xxx/testfile3.py'.replace('/', os.path.sep)])
        ...
        self.assertEqual(
            found,
            ['./test2/testinner/testfile2.py'.replace('/', os.path.sep)])
...
class WriteReformattedCodeTest(yapf_test_helper.YAPFTest):
    ...
    def test_write_to_file(self):
        ...
        with utils.NamedTempFile(dirname=self.test_tmpdir) as (f, fname):
            file_resources.WriteReformattedCode(fname,
                                                s,
                                                in_place=True,
                                                encoding='utf-8')
            f.flush()
