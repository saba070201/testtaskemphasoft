  ...
  """
    if isinstance(node, pytree.Leaf):
        ...
        return fmt.format(name=NodeName(node),
                          ...
                          penalty=GetNodeAnnotation(node,
                                                    Annotation.SPLIT_PENALTY,
                                                    None))
    else:
        fmt = '{node} [{len} children] [child_indent="{indent}"]'
        return fmt.format(node=NodeName(node),
                          len=len(node.children),
                          indent=GetNodeAnnotation(node,
                                                   Annotation.CHILD_INDENT))
...
def IsCommentStatement(node):
    return (NodeName(node) == 'simple_stmt'
            and node.children[0].type == token.COMMENT)
