  ...
  """
    ...
    """
        for child in node.children:
            ...
            # set of nodes.
            if (child.type == grammar_token.NAME
                    and child.value in substatement_names):
                self._StartNewLine()
            self.Visit(child)
    ...
    def Visit_decorator(self, node):  # pylint: disable=invalid-name
        for child in node.children:
            self.Visit(child)
            if child.type == grammar_token.COMMENT and child == node.children[
                    0]:
                self._StartNewLine()
    ...
    if not _ContainsComments(node):
        ...
        if token.value == '(':
            if sum(1 for ch in node.children
                   if ch.type == grammar_token.COMMA) < 2:
                return
