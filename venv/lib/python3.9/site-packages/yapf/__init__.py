...
def FormatFiles(filenames,
  ...
  """
    ...
    if parallel:
        ...
        with concurrent.futures.ProcessPoolExecutor(workers) as executor:
            future_formats = [
                executor.submit(_FormatFile, filename, lines, style_config,
                                no_local_style, in_place, print_diff, quiet,
                                verbose, print_modified)
        ...
        for filename in filenames:
            changed |= _FormatFile(filename, lines, style_config,
                                   no_local_style, in_place, print_diff, quiet,
                                   verbose, print_modified)
    return changed
...
def _GetLines(line_strings):
  ...
  """
    ...
    for line_string in line_strings:
        ...
        if line[0] > line[1]:
            raise errors.YapfError('end comes before start in line range: %r' %
                                   line)
...
def _BuildParser():
  ...
  """
    parser = argparse.ArgumentParser(prog='yapf',
