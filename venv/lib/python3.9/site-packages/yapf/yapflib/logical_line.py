  ...
  """
    ...
    def CalculateFormattingInformation(self):
        ...
        for token in self._tokens[1:]:
            ...
            token.can_break_before = (token.must_break_before
                                      or _CanBreakBefore(prev_token, token))
...
def _SpaceRequiredBetween(left, right, is_line_disabled):
    ...
    lval = left.value
    rval = right.value
    if (left.is_pseudo and _IsIdNumberStringToken(right)
    ...
    if style.Get('SPACE_INSIDE_BRACKETS'):
        ...
        if right.ClosesScope() and lval == ':':
            return True
    if (style.Get('SPACES_AROUND_SUBSCRIPT_COLON')
            and (_IsSubscriptColonAndValuePair(left, right)
                 or _IsSubscriptColonAndValuePair(right, left))):
    ...
    if lval == '.' and rval == 'import':
        ...
        return True
    if (lval == '=' and rval in {'.', ',,,'}
            and subtypes.DEFAULT_OR_NAMED_ASSIGN not in left.subtypes):
    ...
    if ((right.is_keyword or right.is_name)
        ...
        return True
    if (subtypes.SUBSCRIPT_COLON in left.subtypes
            or subtypes.SUBSCRIPT_COLON in right.subtypes):
        # A subscript shouldn't have spaces separating its colons.
        return False
    if (subtypes.TYPED_NAME in left.subtypes
            or subtypes.TYPED_NAME in right.subtypes):
        # A typed argument should have a space after the colon.
        return True
    if left.is_string:
        if (rval == '=' and subtypes.DEFAULT_OR_NAMED_ASSIGN_ARG_LIST
                in right.subtypes):
    ...
    if left.is_binary_op or right.is_binary_op:
        ...
        if style.Get('ARITHMETIC_PRECEDENCE_INDICATION'):
            if _PriorityIndicatingNoSpace(left) or _PriorityIndicatingNoSpace(
                    right):
                return False
            else:
                return True
    ...
    if (_IsUnaryOperator(left) and lval != 'not'
        ...
        # the current token.
        return False
    if (subtypes.DEFAULT_OR_NAMED_ASSIGN in left.subtypes
            and subtypes.TYPED_NAME not in right.subtypes):
    ...
    if (subtypes.DEFAULT_OR_NAMED_ASSIGN in right.subtypes
            and subtypes.TYPED_NAME not in left.subtypes):
        # A named argument or default parameter shouldn't have spaces around it.
        return style.Get('SPACES_AROUND_DEFAULT_OR_NAMED_ASSIGN')
    if (subtypes.VARARGS_LIST in left.subtypes
            or subtypes.VARARGS_LIST in right.subtypes):
        return False
    ...
    if not is_line_disabled and (left.OpensScope() or right.ClosesScope()):
        if (style.GetOrDefault('SPACES_AROUND_DICT_DELIMITERS', False) and
            ((lval == '{'
              and _IsDictListTupleDelimiterTok(left, is_opening=True)) or
             (rval == '}'
              and _IsDictListTupleDelimiterTok(right, is_opening=False)))):
            return True
    ...
    if left.OpensScope() and _IsUnaryOperator(right):
        ...
        return style.Get('SPACE_INSIDE_BRACKETS')
    if (left.OpensScope()
            and (subtypes.VARARGS_STAR in right.subtypes
                 or subtypes.KWARGS_STAR_STAR in right.subtypes)):
...
def _MustBreakBefore(prev_token, cur_token):
    ...
    if prev_token.is_comment or (prev_token.previous_token
                                 and prev_token.is_pseudo
                                 and prev_token.previous_token.is_comment):
...
def _CanBreakBefore(prev_token, cur_token):
    ...
    if not style.Get('ALLOW_SPLIT_BEFORE_DEFAULT_OR_NAMED_ASSIGNS'):
        if (subtypes.DEFAULT_OR_NAMED_ASSIGN in cur_token.subtypes
                or subtypes.DEFAULT_OR_NAMED_ASSIGN in prev_token.subtypes):
            return False
    return True
...
def _SplitPenalty(prev_token, cur_token):
    ...
    if (subtypes.COMP_FOR in cur_token.subtypes
            or subtypes.COMP_IF in cur_token.subtypes):
    ...
    if (subtypes.VARARGS_STAR in prev_token.subtypes
            or subtypes.KWARGS_STAR_STAR in prev_token.subtypes):
    ...
    if cval == '=':
        ...
        return split_penalty.UNBREAKABLE
    if (subtypes.DEFAULT_OR_NAMED_ASSIGN in prev_token.subtypes
            or subtypes.DEFAULT_OR_NAMED_ASSIGN in cur_token.subtypes):
