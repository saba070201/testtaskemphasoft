  ...
  """
    ...
    if style.Get('I18N_FUNCTION_CALL'):
        ...
        for index in range(length - 1):
            if (line.tokens[index + 1].value == '('
                    and line.tokens[index].value
                    in style.Get('I18N_FUNCTION_CALL')):
                return True
    return False
...
def _CanPlaceOnSingleLine(line):
  ...
  """
    ...
    if (style.Get('SPLIT_ARGUMENTS_WHEN_COMMA_TERMINATED') and any(
            token_types[token_index - 1] == token.COMMA
    ...
    if last is None:
        return True
    return (last.total_length + indent_amt <= style.Get('COLUMN_LIMIT')
            and not any(tok.is_comment for tok in line.tokens[:last_index]))
        ...
        for tok in line.tokens:
            if (tok.is_comment and isinstance(tok.spaces_required_before, list)
                ...
                while True:
                    ...
                    if (all_pc_line_lengths and this_line.tokens[0].
...
def _FormatFirstToken(first_token, indent_depth, prev_line, final_lines):
  ...
  """
    ...
    first_token.AddWhitespacePrefix(_CalculateNumberOfNewlines(
...
def _IsClassOrDef(tok):
    if tok.value in {'class', 'def', '@'}:
        return True
    return (tok.next_token and tok.value == 'async'
            and tok.next_token.value == 'def')
    ...
    if _IsClassOrDef(first_token):
        ...
        if not indent_depth:
            ...
            is_inline_comment = prev_last_token.whitespace_prefix.count(
                '\n') == 0
            if (not prev_line.disable and prev_last_token.is_comment
    ...
    while index < len(lines):
        if lines[index].disable:
            ...
            while index < len(lines):
                ...
                if line.last.value != ':':
                    leaf = pytree.Leaf(type=token.SEMI,
                                       value=';',
                                       context=('', (line.lineno, column)))
                    line.AppendToken(
                        format_token.FormatToken(leaf,
                                                 pytree_utils.NodeName(leaf)))
            ...
            if (len(next_line.tokens) == 1
                    and next_line.first.is_multiline_string):
