...
class FormatDecisionState(object):
  ...
  """
    ...
    def Clone(self):
        ...
        new.param_list_stack = [
            state.Clone() for state in self.param_list_stack
        ]
        return new
    ...
    def __eq__(self, other):
        ...
        return (self.next_token == other.next_token
                ...
                and self.line.depth == other.line.depth
                and self.lowest_level_on_line == other.lowest_level_on_line
                and (self.ignore_stack_for_comparison
                     or other.ignore_stack_for_comparison or self.stack
                     == other.stack and self.comp_stack == other.comp_stack
                     and self.param_list_stack == other.param_list_stack))
    ...
    """
        ...
        if (not must_split and subtypes.DICTIONARY_KEY_PART in current.subtypes
                and subtypes.DICTIONARY_KEY not in current.subtypes
        ...
        if (not must_split and subtypes.DICTIONARY_VALUE in current.subtypes
    ...
    def MustSplit(self):
        ...
        if (style.Get('FORCE_MULTILINE_DICT')
                and subtypes.DICTIONARY_KEY in current.subtypes
        ...
        if (self.stack[-1].split_before_closing_bracket and
            (current.value in '}]'
             and style.Get('SPLIT_BEFORE_CLOSING_BRACKET') or
            ...
            if (subtypes.SUBSCRIPT_BRACKET not in current.subtypes
                    or (previous.value == ','
                        and not style.Get('DISABLE_ENDING_COMMA_HEURISTIC'))):
        ...
        if (style.Get('DEDENT_CLOSING_BRACKETS')
                or style.Get('INDENT_CLOSING_BRACKETS')
            ...
            if subtypes.SUBSCRIPT_BRACKET not in bracket.subtypes:
                if bracket.OpensScope():
                    ...
                    if (not _IsLastScopeInLine(bracket)
                            or logical_line.IsSurroundedByBrackets(bracket)):
                ...
                elif (style.Get('DEDENT_CLOSING_BRACKETS')
                      or style.Get('INDENT_CLOSING_BRACKETS')
                      ) and current.ClosesScope():
        ...
        if (style.Get('SPLIT_BEFORE_EXPRESSION_AFTER_OPENING_PAREN')
            ...
            if (previous.value == '(' and not previous.is_pseudo
                    and not logical_line.IsSurroundedByBrackets(previous)):
        ...
        if (style.Get('SPLIT_BEFORE_DICT_SET_GENERATOR')
                and subtypes.DICT_SET_GENERATOR in current.subtypes):
        ...
        if (style.Get('SPLIT_BEFORE_NAMED_ASSIGNS') and not current.is_comment
                and subtypes.DEFAULT_OR_NAMED_ASSIGN_ARG_LIST
                in current.subtypes):
            if (previous.value not in {'=', ':', '*', '**'}
                ...
                if previous.value == '(':
                    ...
                    #      c=2)
                    if (self._FitsOnLine(previous, previous.matching_bracket)
                            and logical_line.IsSurroundedByBrackets(previous)):
        ...
        if (previous.OpensScope() and not current.OpensScope()
                and not current.is_comment
                and subtypes.SUBSCRIPT_BRACKET not in previous.subtypes):
    ...
    """
        ...
        if not dry_run:
            ...
            current.AddWhitespacePrefix(newlines_before=1,
                                        spaces=spaces,
    ...
    """
        ...
        if current.OpensScope():
            last = self.stack[-1]
            new_indent = style.Get(
                'CONTINUATION_INDENT_WIDTH') + last.last_space

            self.stack.append(
                _ParenState(new_indent, self.stack[-1].last_space))
            self.paren_level += 1
        ...
        if (not current.is_pylint_comment and not current.is_pytype_comment
                and not current.is_copybara_comment
                and self.column > self.column_limit):
            excess_characters = self.column - self.column_limit
            penalty += style.Get(
                'SPLIT_PENALTY_EXCESS_CHARACTER') * excess_characters
    ...
    """
        ...
        if (subtypes.COMP_EXPR in current.subtypes
                and subtypes.COMP_EXPR not in previous.subtypes):
        ...
        if (subtypes.COMP_IF in current.subtypes
                and subtypes.COMP_IF not in previous.subtypes):
    ...
    """
        ...
        if _IsFunctionDefinition(previous):
            ...
            if not newline:
                ...
                if param_list.parameters and param_list.has_typed_return:
                    ...
                    total_length -= last_param.total_length - len(
                        last_param.value)
        ...
        if current == self.param_list_stack[-1].closing_bracket:
            ...
            if (not newline and param_list.has_typed_return
                    and param_list.has_split_before_first_param):
        ...
        if (not newline and style.Get('SPLIT_BEFORE_NAMED_ASSIGNS')
                and param_list.has_default_values
                and current != param_list.parameters[0].first_token
                and current != param_list.closing_bracket
                and subtypes.PARAMETER_START in current.subtypes):
    ...
    def _IndentWithContinuationAlignStyle(self, column):
        ...
        if align_style == 'VALIGN-RIGHT':
            ...
            return indent_width * int(
                (column + indent_width - 1) / indent_width)
        return column
    ...
    def _GetNewlineColumn(self):
        ...
        if (previous and previous.is_string and current.is_string
                and subtypes.DICTIONARY_VALUE in current.subtypes):
        ...
        if (not self.param_list_stack and _IsCompoundStatement(self.line.first)
        ...
        if (self.param_list_stack
                and not self.param_list_stack[-1].SplitBeforeClosingBracket(
                    top_of_stack.indent) and top_of_stack.indent
                == ((self.line.depth + 1) * style.Get('INDENT_WIDTH'))):
            if (subtypes.PARAMETER_START in current.subtypes
                    or (previous.is_comment
                        and subtypes.PARAMETER_START in previous.subtypes)):
    ...
    def _EachDictEntryFitsOnOneLine(self, opening):
        ...
        while current and current != closing:
            ...
            if current.OpensScope():
                if ((current.value == '{' or
                     (current.is_pseudo and current.next_token.value == '{')
                     and subtypes.DICTIONARY_VALUE in current.subtypes)
...
def _IsSingleElementTuple(token):
    ...
    while token != close:
        if token.value == ',':
            num_commas += 1
        token = token.matching_bracket if token.OpensScope(
