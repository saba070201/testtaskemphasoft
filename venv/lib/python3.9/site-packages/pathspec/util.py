...
import sys
import warnings
from collections.abc import (Collection as CollectionType, Iterable as
                             IterableType)
from os import (PathLike)
from typing import (Any, AnyStr, Callable, Collection, Dict, Iterable,
                    Iterator, List, Optional, Sequence, Set, Union)
    ...
    for pattern in patterns:
        if pattern.include is not None:
            result_files = pattern.match(
                all_files)  # TODO: Replace with `.match_file()`.
            if pattern.include:
...
def _is_iterable(value: Any) -> bool:
	...
	*value* is the value to check,
    ...
    return isinstance(value,
                      IterableType) and not isinstance(value, (str, bytes))
...
def iter_tree_entries(
	...
	*follow_links* (:class:`bool` or :data:`None`) optionally is whether
    ...
    yield from _iter_tree_entries_next(os.path.abspath(root), '', {}, on_error,
                                       follow_links)
    ...
    with os.scandir(dir_full) as scan_iter:
        ...
        for node_ent in scan_iter:
            ...
            if node_ent.is_dir(follow_symlinks=follow_links):
                ...
                yield from _iter_tree_entries_next(root_full, node_rel, memo,
...
def register_pattern(
	...
	*override* (:class:`bool` or :data:`None`) optionally is whether to
    ...
    if not callable(pattern_factory):
        raise TypeError(
            f"pattern_factory:{pattern_factory!r} is not callable.")
