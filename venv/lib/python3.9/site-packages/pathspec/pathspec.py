...
from typing import (AnyStr, Callable, Collection, Iterable, Iterator, Optional,
                    Type, TypeVar, Union)

from . import util
from .pattern import (Pattern)
from .util import (StrPath, TreeEntry, _filter_patterns, _is_iterable,
                   match_file, normalize_file)
...
class PathSpec(object):
    ...
    def __init__(self, patterns: Iterable[Pattern]) -> None:
		...
		*patterns* (:class:`~collections.abc.Collection` or :class:`~collections.abc.Iterable`)
        ...
        self.patterns = patterns if isinstance(
            patterns, CollectionType) else list(patterns)
        """
    ...
    def match_tree_entries(
		...
		*negate* (:class:`bool` or :data:`None`) is whether to negate the match
		...
		:class:`.TreeEntry`).
		"""
        entries = util.iter_tree_entries(root,
                                         on_error=on_error,
    ...
    def match_tree_files(
		...
		*negate* (:class:`bool` or :data:`None`) is whether to negate the match
		...
		:class:`str`).
		"""
        files = util.iter_tree_files(root,
                                     on_error=on_error,
                                     follow_links=follow_links)
