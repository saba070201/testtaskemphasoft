...
class Sectioned:
    ...
    def section_pairs(cls, text):
        return (section._replace(value=Pair.parse(section.value))
...
class EntryPoint(DeprecatedTuple):
    ...
    pattern = re.compile(r'(?P<module>[\w.]+)\s*'
                         r'(:\s*(?P<attr>[\w.]+)\s*)?'
                         r'((?P<extras>\[.*\])\s*)?$')
    """
    ...
    def __repr__(self):
        return (f'EntryPoint(name={self.name!r}, value={self.value!r}, '
...
class EntryPoints(tuple):
    ...
    def select(self, **params):
        ...
        """
        return EntryPoints(ep for ep in self
                           if _py39compat.ep_matches(ep, **params))
    ...
    @staticmethod
    def _from_text(text):
        return (EntryPoint(name=item.value.name,
...
class PackagePath(pathlib.PurePosixPath):
    ...
    def read_text(self,
                  encoding: str = 'utf-8') -> str:  # type: ignore[override]
...
class DeprecatedNonAbstract:
    def __new__(cls, *args, **kwargs):
        all_names = {
            name
            for subclass in inspect.getmro(cls) for name in vars(subclass)
        }
        abstract = {
            name
...
class Distribution(DeprecatedNonAbstract):
    ...
    def files(self) -> Optional[List[PackagePath]]:
        ...
        return skip_missing_files(
            make_files(self._read_files_distinfo()
    ...
    def _read_files_egginfo_installed(self):
        ...
        paths = ((subdir / name).resolve().relative_to(
            self.locate_file('').resolve()).as_posix()
...
class Lookup:
    ...
    def search(self, prepared):
        infos = (self.infos[prepared.normalized] if prepared else
                 itertools.chain.from_iterable(self.infos.values()))
        eggs = (self.eggs[prepared.legacy_normalized] if prepared else
...
class PathDistribution(Distribution):
    ...
    def _normalized_name(self):
        ...
        return (pass_none(Prepared.normalize)(self._name_from_stem(stem))
...
def entry_points(**params) -> EntryPoints:
    ...
    """
    eps = itertools.chain.from_iterable(dist.entry_points
                                        for dist in _unique(distributions()))
