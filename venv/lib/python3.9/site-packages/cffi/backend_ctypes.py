...
class CTypesData(object):
    ...
    def _make_cmp(name):
        ...
        def cmp(self, other):
            ...
            w_is_ptr = (isinstance(other, CTypesData)
                        and not isinstance(other, CTypesGenericPrimitive))
...
class CTypesBackend(object):
        ...
        class CTypesPtr(CTypesGenericPtr):
            ...
            def __init__(self, init):
                ...
                if kind == 'charp':
                    ...
                    self._as_ctype_ptr = ctypes.cast(self.__as_strbuf,
                                                     self._ctype)
                else:
            ...
            def _get_own_repr(self):
                if getattr(self, '_own', False):
                    return 'owning %d bytes' % (ctypes.sizeof(
                        self._as_ctype_ptr.contents), )
                return super(CTypesPtr, self)._get_own_repr()

        #
        if (BItem is self.ffi._get_cached_btype(model.void_type)
                or BItem is self.ffi._get_cached_btype(
                ...
                if isinstance(init, bytes):
                    ...
                    if isinstance(init, CTypesGenericArray):
                        if (len(init) != len(blob)
                                or not isinstance(init, CTypesArray)):
    ...
    def complete_struct_or_union(self,
                                 CTypesStructOrUnion,
                                 fields,
                                 tp,
                                 totalsize=-1,
        ...
        if self._weakref_cache_ref is None:
            ...
            class MyRef(weakref.ref):
                def __eq__(self, other):
                    myref = self()
                    return self is other or (myref is not None
                                             and myref is other())
