...
USE_LIMITED_API = (sys.platform != 'win32' or sys.version_info < (3, 0)
...
class FieldExpr:
    ...
    def as_field_python_expr(self):
        if self.field_type_op.op == OP_NOOP:
            ...
            raise NotImplementedError
        return "b'%s%s%s'" % (self.field_type_op.as_python_bytes(), size_expr,
                              self.name)
...
class StructUnionExpr:
    ...
    def as_c_expr(self):
        return ('  { "%s", %d, %s,' %
                (self.name, self.type_index, self.flags) + '\n    %s, %s, ' %
                (self.size, self.alignment) + '%d, %d ' %
                (self.first_field_index, len(self.c_fields)) +
    ...
    def as_python_expr(self):
        ...
        return "(b'%s%s%s',%s)" % (format_four_bytes(
            self.type_index), format_four_bytes(flags), self.name,
                                   ','.join(fields_expr))
...
class EnumExpr:
    ...
    def as_c_expr(self):
        return ('  { "%s", %d, _cffi_prim_int(%s, %s),\n'
                '    "%s" },' % (self.name, self.type_index, self.size,
                                 self.signed, self.allenums))
    ...
    def as_python_expr(self):
        ...
        return "b'%s%s%s\\x00%s'" % (format_four_bytes(
            self.type_index), format_four_bytes(prim_index), self.name,
                                     self.allenums)
...
class Recompiler:
    ...
    def collect_type_table(self):
        ...
        for tp in all_decls:
            if tp.is_raw_function:
                ...
                for tp1 in tp.args:
                    assert isinstance(
                        tp1, (model.VoidType, model.BasePrimitiveType,
                              model.PointerType, model.StructOrUnionOrEnum,
    ...
    def _generate(self, step_name):
        ...
        for name, (tp, quals) in sorted(lst):
            ...
            try:
                method = getattr(self,
                                 '_generate_cpy_%s_%s' % (kind, step_name))
            except AttributeError:
                raise VerificationError("not implemented in recompile(): %r" %
                                        name)
            try:
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        extraarg = ''
        if isinstance(tp,
                      model.BasePrimitiveType) and not tp.is_complex_type():
        ...
        #
            ...
            self._convert_funcarg_to_c_ptr_or_array(tp, fromvar, tovar,
                                                    errcode)
            return
        #
        elif (isinstance(tp, model.StructOrUnionOrEnum)
              or isinstance(tp, model.BasePrimitiveType)):
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        def need_indirection(type):
            return (isinstance(type, model.StructOrUnion)
                    or (isinstance(type, model.PrimitiveType)
        ...
        for fname, ftype, fbitsize, fqual in self._enum_fields(tp):
            try:
                ...
                while (isinstance(ftype, model.ArrayType)
                    ...
                    ftype = ftype.item
                    fname = fname + '[0]'
                prnt('  { %s = &p->%s; (void)tmp; }' % (ftype.get_c_name(
                    '*tmp', 'field %r' % fname, quals=fqual), fname))
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                elif cname is None or (isinstance(fldtype, model.ArrayType)
                                       and fldtype.length is None):
                    size = '(size_t)-1'
                else:
                    size = 'sizeof(((%s)0)->%s)' % (tp.get_c_name(
                        '*') if named_ptr is None else named_ptr.name, fldname)
    ...
    def _generate_cpy_const(self,
        ...
        if (category, name) in self._seen_constants:
            raise VerificationError("duplicate declaration of %s '%s'" %
                                    (category, name))
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        if isinstance(tp.result, model.VoidType):
            ...
            context = 'result of %s' % name
            size_of_result = '(int)sizeof(%s)' % (tp.result.get_c_name(
                '', context), )
        ...
        def may_need_128_bits(tp):
            return (isinstance(tp, model.PrimitiveType)
        ...
        if isinstance(tp.result, model.StructOrUnion):
            size_of_a = 'sizeof(%s) > %d ? sizeof(%s) : %d' % (
                tp.result.get_c_name(''), size_of_a, tp.result.get_c_name(''),
                size_of_a)
        prnt('%s%s' %
             (tag_and_space, tp.result.get_c_name(name_and_arguments)))
        prnt('{')
