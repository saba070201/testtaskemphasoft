...
class Factory:
    ...
    def _make_extras_candidate(self, base: BaseCandidate,
                               extras: FrozenSet[str]) -> ExtrasCandidate:
            ...
            if link not in self._editable_candidate_cache:
                try:
                    ...
                    if not self._suppress_build_failures:
                        raise
                    logger.warning("Discarding %s due to build failure: %s",
                                   link, e)
                    self._build_failures[link] = e
                    return None
    ...
    def _iter_found_candidates(
        ...
        def _get_installed_candidate() -> Optional[Candidate]:
            ...
            if not specifier.contains(installed_dist.version,
                                      prereleases=True):
                return None
    ...
    def make_requirement_from_candidate(
            self, candidate: Candidate) -> ExplicitRequirement:
    ...
    def get_wheel_cache_entry(self, link: Link,
                              name: Optional[str]) -> Optional[CacheEntry]:
    ...
    def _report_requires_python_error(
            self,
            causes: Sequence["ConflictCause"]) -> UnsupportedPythonVersion:
