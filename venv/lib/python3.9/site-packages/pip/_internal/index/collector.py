...
class CacheablePageContent:
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other,
                          type(self)) and self.page.url == other.page.url
...
class ParseLinks(Protocol):
    def __call__(self, page: "HTMLPage",
...
def with_cached_html_pages(fn: ParseLinks) -> ParseLinks:
    ...
    def wrapper(cacheable_page: CacheablePageContent,
                use_deprecated_html5lib: bool) -> List[Link]:
        return list(fn(cacheable_page.page, use_deprecated_html5lib))

    @functools.wraps(fn)
    def wrapper_wrapper(page: "HTMLPage",
                        use_deprecated_html5lib: bool) -> List[Link]:
...
class HTMLLinkParser(HTMLParser):
    ...
    def get_href(self, attrs: List[Tuple[str,
...
def _get_html_page(
        link: Link,
        session: Optional[PipSession] = None) -> Optional["HTMLPage"]:
    if session is None:
        raise TypeError(
...
class LinkCollector:
    ...
    def collect_sources(
        ...
        index_url_sources = collections.OrderedDict(
            ...
            ) for loc in self.search_scope.get_index_urls_locations(
                project_name)).values()
        find_links_sources = collections.OrderedDict(
            ...
            ) for loc in self.find_links).values()
