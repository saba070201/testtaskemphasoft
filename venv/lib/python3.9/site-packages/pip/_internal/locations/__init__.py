...
def _should_use_sysconfig() -> bool:
    ...
    """
    return bool(
        getattr(sysconfig, "_PIP_USE_SYSCONFIG", _USE_SYSCONFIG_DEFAULT))
...
def _warn_if_mismatch(old: pathlib.Path, new: pathlib.Path, *,
                      key: str) -> bool:
    if old == new:
        return False
...
def get_bin_prefix() -> str:
    ...
    if _warn_if_mismatch(pathlib.Path(old),
                         pathlib.Path(new),
                         key="bin_prefix"):
        _log_context()
    return old
