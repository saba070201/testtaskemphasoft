...
def _normalized_outrows(
    outrows: Iterable[InstalledCSVRow], ) -> List[Tuple[str, str, str]]:
    ...
    return sorted((record_path, hash_, str(size))
...
def _fs_to_record_path(path: str,
                       relative_to: Optional[str] = None) -> RecordPath:
    if relative_to is not None:
        # On Windows, do not handle relative paths if they belong to different
        # logical disks
        if (os.path.splitdrive(path)[0].lower() == os.path.splitdrive(
                relative_to)[0].lower()):
...
class ZipBackedFile:
    def __init__(self, src_record_path: RecordPath, dest_path: str,
