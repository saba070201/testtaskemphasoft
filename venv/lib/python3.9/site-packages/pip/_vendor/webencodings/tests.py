...
def test_decode():
    ...
    assert decode(b'\xEF\xBB\xBF\xc3\xa9',
                  'ascii') == ('é', lookup('utf8'))  # UTF-8 with BOM

    assert decode(b'\xFE\xFF\x00\xe9',
                  'ascii') == ('é', lookup('utf-16be'))  # UTF-16-BE with BOM
    assert decode(b'\xFF\xFE\xe9\x00',
                  'ascii') == ('é', lookup('utf-16le'))  # UTF-16-LE with BOM
    assert decode(b'\xFE\xFF\xe9\x00',
                  'ascii') == ('\ue900', lookup('utf-16be'))
    assert decode(b'\xFF\xFE\x00\xe9',
                  'ascii') == ('\ue900', lookup('utf-16le'))
...
def test_iter_decode():
    ...
    assert iter_decode_to_string([b'\xEF\xBB\xBF', b'\xc3', b'\xa9'],
                                 'latin1') == 'é'
    assert iter_decode_to_string([b'\xEF\xBB\xBF', b'a', b'\xc3'],
                                 'latin1') == 'a\uFFFD'
    ...
    assert iter_decode_to_string([b'', b'h\xe9', b'llo'],
                                 'x-user-defined') == 'h\uF7E9llo'
...
def test_iter_encode():
    ...
    assert b''.join(iter_encode(['', 'h\uF7E9', '', 'llo'],
                                'x-user-defined')) == b'h\xe9llo'
...
def test_x_user_defined():
    ...
    assert decode(encoded,
                  'x-user-defined') == (decoded, lookup('x-user-defined'))
