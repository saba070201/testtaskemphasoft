    ...
    **Subclassing the HTML formatter**
    ...
    def _wrap_full(self, inner, outfile):
        if self.cssfile:
            ...
            yield 0, (DOC_HEADER_EXTERNALCSS % dict(title=self.title,
                                                    cssfile=self.cssfile,
                                                    encoding=self.encoding))
        else:
            yield 0, (DOC_HEADER % dict(title=self.title,
                                        styledefs=self.get_style_defs('body'),
                                        encoding=self.encoding))
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        yield 0, ('<table class="%stable">' % self.cssclass + filename_tr +
                  '<tr><td class="linenos"><div class="linenodiv"><pre>' + ls +
    ...
    def _wrap_lineanchors(self, inner):
        ...
        for t, line in inner:
            if t:
                ...
                yield 1, '<a id="%s-%d" name="%s-%d"%s></a>' % (s, i, s, i,
                                                                href) + line
            else:
                yield 0, line
    ...
    def _format_lines(self, tokensource):
        ...
        for ttype, value in tokensource:
            try:
                ...
                title = ' title="%s"' % '.'.join(
                    ttype) if self.debug_token_types else ''
                if nocls:
