...
_encoding_map = [(b'\xef\xbb\xbf', 'utf-8'), (b'\xff\xfe\0\0', 'utf-32'),
                 (b'\0\0\xfe\xff', 'utf-32be'), (b'\xff\xfe', 'utf-16'),
                 (b'\xfe\xff', 'utf-16be')]
            ...
            if action is None:
                ...
                if data is not None:
                    ...
                    for item in action(lexer,
                                       _PseudoMatch(match.start(i + 1), data),
                                       ctx):
                        if item:
                            yield item
        if ctx:
    ...
    def _process_state(cls, unprocessed, processed, state):
        ...
        for tdef in unprocessed[state]:
            ...
            if isinstance(tdef, default):
                new_state = cls._process_new_state(tdef.state, unprocessed,
                                                   processed)
            ...
            if len(tdef) == 2:
                ...
                new_state = cls._process_new_state(tdef[2], unprocessed,
                                                   processed)
...
class ProfilingRegexLexer(RegexLexer, metaclass=ProfilingRegexLexerMeta):
    ...
    def get_tokens_unprocessed(self, text, stack=('root', )):
        ...
        data = sorted(((s, repr(r).strip('u\'').replace(
            '\\\\', '\\')[:65], n, 1000 * t, 1000 * t / n)
