...
class Manifest(object):
    ...
    def sorted(self, wantdirs=False):
        ...
        if wantdirs:
            ...
            result |= dirs
        return [
            os.path.join(*path_tuple) for path_tuple in sorted(
                os.path.split(path) for path in result)
        ]
    ...
    def _translate_pattern(self,
        ...
        if prefix is not None:
            ...
            if os.sep == '\\':
                sep = r'\\'
            if _PYTHON_VERSION <= (3, 2):
                pattern_re = '^' + base + sep.join(
                    (prefix_re, '.*' + pattern_re))
            else:
