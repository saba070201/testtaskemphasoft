...
LocalType = Union[NegativeInfinityType,
                  Tuple[Union[SubLocalType, Tuple[SubLocalType, str],
...
def _parse_letter_version(
        letter: str, number: Union[str, bytes,
                                   SupportsInt]) -> Optional[Tuple[str, int]]:
...
def _parse_local_version(local: str) -> Optional[LocalType]:
    ...
    if local is not None:
        return tuple(part.lower() if not part.isdigit() else int(part)
...
def _cmpkey(
    ...
    if local is None:
        ...
        #   match exactly
        _local = tuple((i, "") if isinstance(i, int) else (NegativeInfinity, i)
                       for i in local)
