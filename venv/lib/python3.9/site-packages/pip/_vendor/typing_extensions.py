...
T_contra = typing.TypeVar('T_contra',
                          contravariant=True)  # Ditto contravariant.
            ...
            if self.__type__ is None:
                return cls(typing._type_check(
                    item, f'{cls.__name__[1:]} accepts only single type.'),
                           _root=True)
            raise TypeError(
                f'{cls.__name__[1:]} cannot be further subscripted')
...
_PROTO_WHITELIST = [
    ...
    'Reversible', 'ContextManager', 'AsyncContextManager'
]
        ...
        def __init_subclass__(cls, *args, **kwargs):
            ...
            if '__orig_bases__' in cls.__dict__:
                ...
                for base in cls.__orig_bases__:
                    if (isinstance(base, typing._GenericAlias)
                            and base.__origin__ in (typing.Generic, Protocol)):
                        # for error messages
            ...
            def _proto_hook(other):
                ...
                if not getattr(cls, '_is_runtime_protocol', False):
                    if sys._getframe(2).f_globals['__name__'] in [
                            'abc', 'functools'
                    ]:
                        return NotImplemented
                    raise TypeError(
                ...
                for attr in _get_protocol_attrs(cls):
                    for base in other.__mro__:
                        if attr in base.__dict__:
                            ...
                            break
                        annotations = getattr(base, '__annotations__', {})
                        if (isinstance(annotations, typing.Mapping)
            ...
            for base in cls.__bases__:
                if not (base in (object, typing.Generic)
                        or base.__module__ == 'collections.abc'
                        and base.__name__ in _PROTO_WHITELIST or
    ...
    class _ProtocolMeta(GenericMeta):
                ...
                if gvars is None:
                    ...
                    if not tvarset <= gvarset:
                        s_vars = ", ".join(
                            str(t) for t in tvars if t not in gvarset)
                        s_args = ", ".join(str(g) for g in gvars)
                        cls_name = "Generic" if any(
                            b.__origin__ is typing.Generic
        ...
        def __init__(cls, *args, **kwargs):
            ...
            if not cls.__dict__.get('_is_protocol', None):
                cls._is_protocol = any(
                    b is Protocol or isinstance(b, _ProtocolMeta)
                    and b.__origin__ is Protocol for b in cls.__bases__)
            if cls._is_protocol:
                for base in cls.__mro__[1:]:
                    if not (base in (object, typing.Generic)
                            or base.__module__ == 'collections.abc'
                            and base.__name__ in _PROTO_WHITELIST
                            or isinstance(base, typing.TypingMeta) and
                            base._is_protocol or isinstance(base, GenericMeta)
        ...
        def __subclasscheck__(self, cls):
            ...
            if (self.__dict__.get('_is_protocol', None)
                    and not self.__dict__.get('_is_runtime_protocol', None)):
                if sys._getframe(1).f_globals['__name__'] in [
                        'abc', 'functools', 'typing'
                ]:
                    return False
                raise TypeError(
                    "Instance and class checks can only be used with"
                    " @runtime protocols")
            if (self.__dict__.get('_is_runtime_protocol', None)
        ...
        def __getitem__(self, params):
            ...
            if self in (typing.Generic, Protocol):
                ...
                raise TypeError(
                    f"Cannot subscript already-subscripted {repr(self)}")
            else:
...
if sys.version_info >= (3, 9, 2):
    ...
    def _typeddict_new(*args, total=True, **kwargs):
        ...
        _, args = args[0], args[1:]  # allow the "cls" keyword be passed
        if args:
            typename, args = args[0], args[
                1:]  # allow the "_typename" keyword be passed
        ...
        try:
            ...
            ns['__module__'] = sys._getframe(1).f_globals.get(
                '__name__', '__main__')
        except (AttributeError, ValueError):
            pass
...
# 3.7-3.8
    ...
    class _AnnotatedAlias(typing._GenericAlias, _root=True):
        ...
        def __reduce__(self):
            return operator.getitem, (Annotated,
...
# 3.9
    ...
    def TypeGuard(self, parameters):
        ...
        item = typing._type_check(parameters,
                                  f'{self} accepts only single type.')
        return typing._GenericAlias(self, (item, ))
# 3.7-3.8
elif sys.version_info[:2] >= (3, 7):
