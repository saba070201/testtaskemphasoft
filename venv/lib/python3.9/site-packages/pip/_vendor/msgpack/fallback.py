...
if sys.version_info < (3, 5):
    ...
    def _is_recursionerror(e):
        return (len(e.args) == 1 and isinstance(e.args[0], str)
...
class Unpacker(object):
    ...
    def feed(self, next_bytes):
        ...
        if len(self._buffer) - self._buff_i + len(
                view) > self._max_buffer_size:
            raise BufferFull
    ...
    def _read_header(self):
        ...
        if b & 0b10000000 == 0:
            ...
            if n > self._max_str_len:
                raise ValueError("%s exceeds max_str_len(%s)" %
                                 (n, self._max_str_len))
            obj = self._read(n)
            ...
            if n > self._max_array_len:
                raise ValueError("%s exceeds max_array_len(%s)" %
                                 (n, self._max_array_len))
        elif b & 0b11110000 == 0b10000000:
            n = b & 0b00001111
            typ = TYPE_MAP
            if n > self._max_map_len:
                raise ValueError("%s exceeds max_map_len(%s)" %
                                 (n, self._max_map_len))
        elif b == 0xC0:
            obj = None
        elif b == 0xC2:
        ...
        if typ == TYPE_MAP:
            ...
            if self._object_pairs_hook is not None:
                ...
                for _ in xrange(n):
                    ...
                    if self._strict_map_key and type(key) not in (unicode,
                                                                  bytes):
                        raise ValueError("%s is not allowed for map key" %
                                         str(type(key)))
    ...
    def _pack(
        ...
        while True:
            ...
            if self._datetime and check(obj, _DateTime):
                raise ValueError("Cannot serialize %r where tzinfo=None" %
                                 (obj, ))
