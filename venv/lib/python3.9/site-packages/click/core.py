...
def _complete_visible_commands(
        ctx: "Context",
        incomplete: str) -> t.Iterator[t.Tuple[str, "Command"]]:
...
def _check_multicommand(base_command: "MultiCommand",
                        cmd_name: str,
                        cmd: "Command",
...
def augment_usage_errors(
        ctx: "Context",
        param: t.Optional["Parameter"] = None) -> t.Iterator[None]:
...
class Context:
    ...
    def __init__(
        ...
        self.token_normalize_func: t.Optional[t.Callable[
            [str], str]] = token_normalize_func
    ...
    def make_formatter(self) -> HelpFormatter:
        ...
        """
        return self.formatter_class(width=self.terminal_width,
    ...
    def call_on_close(self, f: t.Callable[...,
    ...
    def lookup_default(self,
                       name: str,
                       call: "te.Literal[True]" = True) -> t.Optional[t.Any]:
        ...
        ...
        while ctx.parent is not None:
            ...
            if isinstance(ctx.command, MultiCommand) and ctx.command.chain:
                results.extend(
                    ...
                    for name, command in _complete_visible_commands(
                        ctx, incomplete) if name not in ctx.protected_args)
...
class Command(BaseCommand):
    ...
    def shell_complete(self, ctx: Context,
        ...
        if incomplete and not incomplete[0].isalnum():
            for param in self.get_params(ctx):
                if (not isinstance(param, Option) or param.hidden or
                    (not param.multiple
...
class MultiCommand(Command):
    ...
    def resolve_command(
        ...
        if cmd is None and not ctx.resilient_parsing:
            ...
            ctx.fail(
                _("No such command {name!r}.").format(name=original_cmd_name))
...
class Group(MultiCommand):
    ...
    def __init__(
        self,
        name: t.Optional[str] = None,
        commands: t.Optional[t.Union[t.MutableMapping[str, Command],
                                     t.Sequence[Command]]] = None,
        **attrs: t.Any,
    ) -> None:
        super().__init__(name, **attrs)
    ...
    def command(
            self, *args: t.Any,
            **kwargs: t.Any) -> t.Callable[[t.Callable[..., t.Any]], Command]:
        ...
    ...
    def group(
            self, *args: t.Any,
            **kwargs: t.Any) -> t.Callable[[t.Callable[..., t.Any]], "Group"]:
        ...
...
class Parameter:
    ...
    def __init__(
        ...
        callback: t.Optional[t.Callable[[Context, "Parameter", t.Any],
                                        t.Any]] = None,
    ...
    def consume_value(
            self, ctx: Context,
            opts: t.Mapping[str, t.Any]) -> t.Tuple[t.Any, ParameterSource]:
    ...
    def type_cast_value(self, ctx: Context, value: t.Any) -> t.Any:
        ...
        def check_iter(value: t.Any) -> t.Iterator[t.Any]:
            try:
                ...
                # the command line.
                raise BadParameter(_("Value must be an iterable."),
                                   ctx=ctx,
                                   param=self) from None
        ...
        if self.nargs == 1 or self.type.is_composite:
            ...
            def convert(value: t.Any) -> t.Any:  # t.Tuple[t.Any, ...]
                return tuple(
                    self.type(x, self, ctx) for x in check_iter(value))
    ...
    def handle_parse_result(self, ctx: Context, opts: t.Mapping[str, t.Any],
                            args: t.List[str]) -> t.Tuple[t.Any, t.List[str]]:
...
class Option(Parameter):
    ...
    def _parse_decls(
        ...
        if name is None and possible_names:
            possible_names.sort(
                key=lambda x: -len(x[0]))  # group long options first
    ...
    def get_help_record(self, ctx: Context) -> t.Optional[t.Tuple[str, str]]:
        ...
        if self.show_envvar:
            ...
            if envvar is not None:
                var_str = (envvar if isinstance(envvar, str) else ", ".join(
                    str(d) for d in envvar))
        ...
        if show_default_is_str or (show_default and
            ...
            if show_default_is_str:
                ...
                default_string = split_opt(
                    (self.opts if self.default else self.secondary_opts)[0])[1]
            ...
            if default_string:
                extra.append(
                    _("default: {default}").format(default=default_string))

        if (isinstance(self.type, types._NumberRangeBase)
                # skip count with default range type
                and not (self.count and self.type.min == 0
    ...
    def consume_value(
            self, ctx: Context,
            opts: t.Mapping[str,
                            "Parameter"]) -> t.Tuple[t.Any, ParameterSource]:
        ...
        elif (self.multiple and value is not None
        ...
        elif (source in {None, ParameterSource.DEFAULT}
