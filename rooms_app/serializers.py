...
class AddBookingSerializer(serializers.ModelSerializer):
    ...
    def get_validation(self, user):
        if self.validated_data["start_booking"] >= self.validated_data["end_booking"]:
            raise serializers.ValidationError()
        if self.Meta.model.objects.filter(
            (
                models.Q(start_booking__lte=self.validated_data["start_booking"])
                and models.Q(end_booking__gt=self.validated_data["start_booking"])
            )
            or models.Q(end_booking__gte=self.validated_data["end_booking"])
            and models.Q(start_booking__lt=self.validated_data["end_booking"]).filter(
